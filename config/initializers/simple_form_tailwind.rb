...
SimpleForm.setup do |config|
  ...
  config.wrappers(:vertical_form, tag: "div", class: "mb-4") do |b|
    ...
    b.use(
      :input,
      class: "shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 bg-white focus:outline-none focus:ring-0 focus:border-blue-500 text-gray-700 leading-6 transition-colors duration-200 ease-in-out",
  ...
  config.wrappers(:vertical_file, tag: "div", class: "") do |b|
    ...
    b.use(
      ...
      error_class: "text-red-700 border-red-500",
      valid_class: "text-green-400",
    )
    b.use(:error, wrap_with: { tag: "p", class: "mt-2 text-red-700 text-xs italic" })
    b.use(:hint, wrap_with: { tag: "p", class: "mt-2 text-grey-700 text-xs italic" })
  ...
  config.wrappers(:vertical_multi_select, tag: "div", class: "my-4", error_class: "f", valid_class: "") do |b|
    ...
    b.wrapper(tag: "div", class: "inline-flex space-x-1") do |ba|
      ba.use(
        :input,
        class: "flex w-auto w-auto text-gray-500 text-sm border-gray-300 rounded p-2",
        error_class: "text-red-700",
        valid_class: "text-green-400",
      )
  ...
  config.wrappers(
    ...
    b.wrapper(tag: "div", class: "flex items-center h-5") do |ba|
      ba.use(
        :input,
        class: "rounded-lg overflow-hidden appearance-none bg-gray-400 h-3 w-full text-gray-300",
        error_class: "text-red-700",
        valid_class: "text-green-400",
      )
    end
