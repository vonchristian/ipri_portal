...
RSpec.describe(Spreadsheets::DraftCaseImports::IndividualVictim, type: :service) do
  let(:draft_case_detail) { create(:draft_case_detail, age_bracket: nil) }
  let!(:age_bracket) { create(:age_bracket, name: "C") }
  let(:case_data) do
    {
   ...
   "The information in this case factsheet can be used by IPRI in understanding broad trends of criminalization, violence and impunity against Indigenous Peoples and can specifically refer to any of the details of this case.",
      ...
      "Date of Birth (Write YYYY-MM-DD) (If more than one individual, list Date of Birth according to the sequence of Name listed.)" => "1990-01-01",
      ...
      "Write the number of 0-12 years old from the total number of individuals affected:" => nil,
  ...
  describe "#execute" do
    ...
    it { expect(draft_case_detail.reload.age_bracket).to(eq(age_bracket)) }
    it { expect(draft_case_detail.reload.full_name).to(eq("Chain Singh")) }
    it { expect(draft_case_detail.reload.gender).to(eq("male")) }
    it { expect(draft_case_detail.reload.dependent_type).to(eq("with_dependents")) }
    it { expect(draft_case_detail.reload.ethnic_identity).to(eq("Bhil")) }
    it { expect(draft_case_detail.reload.date_of_birth_month).to(eq("01")) }
    it { expect(draft_case_detail.reload.date_of_birth_day).to(eq(1)) }
    it { expect(draft_case_detail.reload.date_of_birth_year).to(eq(1990)) }
    it { expect(draft_case_detail.reload.victim_role).to(eq("Community leader")) }
    it { expect(draft_case_detail.reload.dependent_details).to(eq("wives and chidlren")) }
  end
end
