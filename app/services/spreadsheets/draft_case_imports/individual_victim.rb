...
module Spreadsheets
  module DraftCaseImports
    class IndividualVictim < ActiveInteraction::Base
      ...
      def execute
        full_date_of_birth = case_data["Date of Birth (Write YYYY-MM-DD) (If more than one individual, list Date of Birth according to the sequence of Name listed.)"].to_s.split("-")
        draft_case_detail.update(
          ...
          date_of_birth_month: full_date_of_birth.second,
          date_of_birth_day:  full_date_of_birth.last,
          date_of_birth_year:  full_date_of_birth.first,
      ...
      def full_name
        case_data["Name/s (Write Full Name)"] ||
          case_data["Name/s (Write Full Name) (If more than one individual, indicate a number for each name, e.g. 1. John Doe ; 2. Jane Doe.)"]
      end
    end
  end
end
